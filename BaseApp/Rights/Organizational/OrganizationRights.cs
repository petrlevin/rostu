...
namespace BaseApp.Rights.Organizational
{
    ...
    public class OrganizationRights
    {
        ...
        private void ValidateWrite(int[] itemId, string operation)
        {
            ...
            ValidateWrite(operation,new OrganizationRightsSelectQueryBuilder<ImplementationRevert>(_source, itemId, rightData));
        }
        ...
        private void ValidateWrite(Dictionary<string, object> values, int? itemId, string operationName)
        {
            ...
            if (itemId.HasValue)
            {
                ValidateWrite(operationName, new OrganizationRightsSelectQueryBuilder<ImplementationRevert>(_source, itemId.Value, new RightsData(rightsWithSl, WithoutSelfLinked(rightsWithSl.Single(g => g.Key.Id == _source.Id)), extensions)));
                ValidateWrite(operationName, new OrganizationRightsSelectQueryBuilder<ImplementationRevertStrict>(_source, itemId.Value, new RightsData(rightsWithSl, _source, extensions), values));
            }
            else
            {
                ...
                ValidateWrite(operationName, new OrganizationRightsSelectQueryBuilder<ImplementationRevertStrict>(_source, caption.ToString(), new RightsData(rightsWithSl,WithoutId(rightsWithSl.Single(g => g.Key.Id == _source.Id)),extensions), values));
            }
        ...
        private void ValidateWrite(string operationName, OrganizationRightsSelectQueryBuilder builder)
        {
        ...
        private string GetViolationFormat(SqlDataReader reader, OrganizationRightsSelectQueryBuilder builder)
        {
        ...
        private IEnumerable<ViolationInfo> GetViolations(SqlDataReader reader, OrganizationRightsSelectQueryBuilder builder)
        {
            if (!reader.HasRows)
                return null;
            return reader.AsEnumerable(r => new ViolationInfo()
                                                          {
                                                              ...
                                                              For = r.GetString(OrganizationRightsSelectQueryBuilder.CaptionAlias),
