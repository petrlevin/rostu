...
namespace Platform.Web.Services.DataProcessingService
{
    class RoundDocs
    {
        ...
        private LimitBudgetAllocations CloneLimitBudgetAllocations(DataContext context, LimitBudgetAllocations doc)
        {
            ...
            if (!newDoc.IdSBP_BlankActual.HasValue)
            {
                var idchekblanktype = doc.SBP.SBPType == Sbor.DbEnums.SBPType.GeneralManager ? (byte)Sbor.DbEnums.BlankType.BringingGRBS : (byte)Sbor.DbEnums.BlankType.BringingRBS;
                var idcheksbp = doc.SBP.SBPType == Sbor.DbEnums.SBPType.GeneralManager ? doc.IdSBP : doc.IdParent;
        ...
        private ActivityOfSBP CloneActivityOfSBP(DataContext context, ActivityOfSBP doc)
        {
            ...
            foreach (var bgt in budgets)
            {
                ...
                if (!isExistsBlank) // обновляем только если не было бланка !!!
                {
                    var newBlank = context.SBP_BlankHistory.Where(r =>
                        (
                            (doc.SBP.SBPType == Sbor.DbEnums.SBPType.GeneralManager && r.IdOwner == doc.SBP.Id)
                            || (doc.SBP.SBPType == Sbor.DbEnums.SBPType.Manager && r.IdOwner == doc.SBP.IdParent)
                        )
                        && r.IdBlankType == (byte)Sbor.DbEnums.BlankType.FormationGRBS
                        && r.IdBudget == bgt.Id
                    ).OrderByDescending(o => o.DateCreate).FirstOrDefault();
        ...
        public void RoundActivityOfSBP(int topDocs = -1, int idSbp = 0, int idSession = 1, bool isMandatoryOnly = false, int length = -2)
        {
            ...
            using (new ControlScope(new NoControlsStrategy(), false))
            {
                ...
                {
                    using (DataContext context1 = new Tests.DataContext())
                    {
                        ((BaseAppEnvironment) HttpContext.Current.Application["PlatformEnvironment"]).RequestStorage.DbContext = context1;
                        var manager = DataManagerFactory.Create<ToolsDataManager>(context1.Entity.Single(s => s.Id == entity.Id));
