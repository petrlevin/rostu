...
namespace Tools.MigrationHelper.EnumsProcessing
{
    ...
    public class EnumsMerger
    {
        ...
        public EnumsMerger(DbDataSet metadata)
        {
            this.Meta = metadata;
        }
        ...
        /// </summary>
        public DbDataSet Meta { get; private set; }
        ...
        public void Merge(List<DataTable> enumsTables)
        {
            ...
            foreach (DataRow enumInfo in enumsEntityList.Rows)
            {
                ...
                if (entityIdFromMetadata == null)
                {
                    ...
                    newRow[Names.AllowGenericLinks] = false;
	    ...
	    private void checkEnumFields()
	    {
			...
			{
                throw new InvalidOperationException("Ошибка! В xml файлах, для сущностей типа перечисление, не хватает строк в справочнике Поля сущности. Ожидалось: " + fieldsCount * entities.Count() + " В xml файлах: " + entityFieldsCount);
			}
	    }
		...
		private void CheckDataSet(List<DataTable> enumsTables)
        {
            ...
            {
                throw new InvalidOperationException("Ошибка! Найдены перечисления по которым есть запись в Entity.xml, а в коде отсутсвует. Возможно перечисление в КОДЕ было удалено или переименовано. Перечисления:" + mess);
            }
        }
